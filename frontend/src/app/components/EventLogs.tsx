...
export function EventLogs() {
  const [depositevents, depositsetEvents] = useState<any[]>([]);
  const [redeemevents, redeemsetEvents] = useState<any[]>([]);
  ...
  useWatchContractEvent({
    ...
    onLogs(logs) {
      logs.forEach((log) => {
        depositsetEvents((prev) => [log, ...prev].slice(0, 5));
      });
    },
  });
  ...
  useWatchContractEvent({
    ...
    onLogs(logs) {
      logs.forEach((log) => {
        redeemsetEvents((prev) => [log, ...prev].slice(0, 5));
      });
    },
  });
